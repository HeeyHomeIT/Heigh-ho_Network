    ...
    public function orderDetailsToShow()
    {
        ...
        foreach($select as $key=>$val){
            ...
            if($val->order_step==11){
                $material_pay_status=DB::select('select pay_status from hh_order_material where order_id=? and material_type=?',[$order_id,3]);
                $select[$key]->material_pay_status=$material_pay_status[0]->pay_status;
            }
            if($val->order_step==15){
                $material_pay_status=DB::select('select pay_status from hh_order_material where order_id=? and material_type=?',[$order_id,5]);
                $select[$key]->material_pay_status=$material_pay_status[0]->pay_status;
            }
