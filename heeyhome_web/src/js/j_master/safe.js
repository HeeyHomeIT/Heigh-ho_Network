                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.code == '130') {//正在审核中
                                ...
                                $('#new_info').append('<div id="face" class="showImg showFace fl"><img src=""> </div><!--showImg--><div id="back" class="showImg showFace fl"><img src=""></div><!--showImg--><div class="deal_wrap clearfix fl"> <div class="deal_left fl"> <i class="iconfont">&#xe67c;</i> </div><!--deal_left--> <div class="deal_right fl"> <p class="process">身份正在验证中...</p> <p class="wait">请耐心等待</p> </div><!--deal_left--> </div><!--deal_wrap-->');
                                $('.content_wrap').css('height', '898px');
                                $('#face img').attr('src', '' + data.data.facephoto);
                                $('#back img').attr('src', '' + data.data.backphoto);
                            } else if (data.code == '131') {//审核未通过
                                $('.revise_process').hide();
                                $('.bind_phonecnt').hide();
                                $('#new_info').append('<div id="wrap"><div id="face" class="showImg showFace fl"><img src=""> </div><!--showImg--><div id="back" class="showImg showFace fl"><img src=""></div><!--showImg--><div class="deal_wrap clearfix fl"> <div class="deal_left fl"> <i class="iconfont">&#xe625;</i> </div><!--deal_left--> <div class="deal_right fl"> <p class="process">您的审核尚未通过！</p> <p class="wait">请点击<a id="new_upload" href="javascript:;">重新上传信息~~</a></p> </div><!--deal_left--> </div><!--deal_wrap--></div>');
                                $('.content_wrap').css('height', '898px');
                                $('#face img').attr('src', '' + data.data.facephoto);
                                $('#back img').attr('src', '' + data.data.backphoto);
                                ...
                                $('.revise_process').remove();
                                $('.bind_phonecnt').remove();
                                $('#new_info').append('<div id="face" class="showImg showFace fl"><img src=""> </div><!--showImg--><div id="back" class="showImg showFace fl"><img src=""></div><!--showImg--><div class="deal_wrap clearfix fl"> <div class="deal_left fl"> <i class="iconfont">&#xe615;</i> </div><!--deal_left--> <div class="deal_right fl"> <p class="process complete_process">您的审核已通过！</p></div><!--deal_left--> </div><!--deal_wrap-->');
                                $('.content_wrap').css('height', '898px');
                                $('#face img').attr('src', '' + data.data.facephoto);
                                $('#back img').attr('src', '' + data.data.backphoto);
                            } else {
        ...
        initPasswordAuthEvent: function () {
            ...
            HHIT_CENTERAPP.controller('reset1', ['$scope', '$state', function ($scope, $state) {
                ...
                $(document).off('click', '#message').on('click', '#message', function () {//短信验证码方式
                    window.location.href = '#/master/setting/reset/reset_2';
                });
                $(document).off('click', '#password').on('click', '#password', function () {//登录密码方式
                    window.location.href = '#/master/setting/reset/reset_3';
                });
            }]);
            /* 修改密码第一步短信验证身份 */
            HHIT_CENTERAPP.controller('reset2', ['$scope', '$state', function ($scope, $state) {
                var password_phone = function () {
                    ...
                    $(document).off('click', '#re_select').on('click', '#re_select', function () {//跳回到选择何种方式页面
                        window.location.href = '#/master/setting/reset/reset_1';
                    });
                    /* 短信身份验证 */
